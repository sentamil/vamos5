  ...
  //$scope.locations01 = vamoservice.getDataCall($scope.url);
    ...
    function webCall(){
      ...
      if((checkXssProtection($scope.uiDate.fromdate) == true) && ((checkXssProtection($scope.uiDate.fromtime) == true) && (checkXssProtection($scope.uiDate.todate) == true) && (checkXssProtection($scope.uiDate.totime) == true))) {
         ...
         // var siteAlertUrl = 'http://128.199.159.130:9000/getSiteStoppageAlertReport?groupId=vantec:SMP&userId=vantec_user&fromTimeUtc=1500834600000&toTimeUtc=1501007400000';

            var siteAlertUrl = GLOBAL.DOMAIN_NAME+'/getSiteStoppageAlertReport?groupId='+$scope.gName+'&fromTimeUtc='+utcFormat($scope.uiDate.fromdate,convert_to_24h($scope.uiDate.fromtime))+'&toTimeUtc='+utcFormat($scope.uiDate.todate,convert_to_24h($scope.uiDate.totime));
       }

        console.log(siteAlertUrl);
        $scope.siteAlertData=[];

     $http.get(siteAlertUrl).success(function(data){

            $scope.siteAlertData=data;
            console.log($scope.siteAlertData);
